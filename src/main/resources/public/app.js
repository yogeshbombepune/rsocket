(function() { function r(e, n, t) { function o(i, f) { if (!n[i]) { if (!e[i]) { var c = "function" == typeof require && require; if (!f && c) return c(i, !0); if (u) return u(i, !0); var a = new Error("Cannot find module '" + i + "'"); throw a.code = "MODULE_NOT_FOUND", a } var p = n[i] = { exports: {} }; e[i][0].call(p.exports, function(r) { var n = e[i][1][r]; return o(n || r) }, p, p.exports, r, e, n, t) } return n[i].exports } for (var u = "function" == typeof require && require, i = 0; i < t.length; i++)o(t[i]); return o } return r })()({
	1: [function(require, module, exports) {
		...
		function reloadMessages(message) {
			$("#holdings").html("");
			var data = message.stocks;
			//var stockList = data.stocks;
			data.forEach(function(item) {
				var tr = $("<tr />");
				tr.append("<td>" + item.stockName + "</td>");
				tr.append("<td>" + Number(item.quantity).toFixed(0) + "</td>");
				tr.append("<td>" + Number(item.buyPrice).toFixed(2) + " (" + item.targetCurrency + ")" + "</td>");
				tr.append("<td>" + Number(item.marketPrice).toFixed(2) + " (" + item.targetCurrency + ")" + "</td>");

				var td = $("<td />");
				var num = parseFloat(item.profitOrLoss);
				if (num < 0) {
					td.addClass("negative");
				} else if (num > 0) {
					td.addClass("positive");
				}
				td.text(Number(item.profitOrLoss).toFixed(2) + " (" + item.referenceCurrency + ")");
				// tr.append("<td>" + Number(item.profitOrLoss).toFixed(2) + " (" + item.referenceCurrency + ")" + "</td>");
				tr.append(td);
				$("#holdings").append(tr);
			})
		}
		...
		function main() {
			...
			client = new RSocketClient({
				...
				transport: new RSocketWebSocketClient({
					url: 'ws://localhost:7000/holding-socket'
				}),
			});
			...
			client.connect().subscribe({
				onComplete: socket => {
					...
					socket.requestStream({
						...
						metadata: String.fromCharCode('holdings-by-user'.length) + 'holdings-by-user',
					}).subscribe({
		...
		document.getElementById('user-filter').addEventListener('change', main);
